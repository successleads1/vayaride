...
/**
 * Admin → send arbitrary email to drivers (single / many)
 * @param {{ to:string[], subject:string, html?:string, text?:string }} opts
 */
export async function sendAdminEmailToDrivers({ to = [], subject = '', html = '', text = '' } = {}) {
  const uniq = Array.from(new Set((to || []).filter(Boolean)));
  if (uniq.length === 0) {
    console.warn('❌ sendAdminEmailToDrivers: empty recipients, skipping.');
  ...
  if (!html && !text) {
    throw new Error('Provide html or text body');
  }

  // Send individually (clear bounces/reporting)
  ...
  console.log(`📨 Admin bulk mail done -> sent ${sent}/${uniq.length}`);
  return { sent, results };
}
