...
const DriverSchema = new mongoose.Schema({
  ...
  /* ---------- Trip limit controls (NEW) ---------- */
  // Per-method limits (3 by default for both)
  cashTripLimit:    { type: Number, default: 3 },
  payfastTripLimit: { type: Number, default: 3 },

  // Offsets let us “reset” without changing historical rides or recomputed stats
  cashTripOffset:    { type: Number, default: 0 },
  payfastTripOffset: { type: Number, default: 0 },

  // Temporary global override: allow trips even if above limit (applies to both methods)
...
function effectiveCountFrom(doc, method) {
  const raw = Number(
    method === 'cash'
      ? (doc?.stats?.cashCount || 0)
      : (doc?.stats?.payfastCount || 0)
  );
  const off = Number(
    method === 'cash'
      ? (doc?.cashTripOffset || 0)
      : (doc?.payfastTripOffset || 0)
  );
...
DriverSchema.methods.isMethodAllowedNow = function (method /* 'cash'|'payfast' */) {
  const now = new Date();
  const overrideActive = !!this.tripOverride && (!this.tripOverrideExpiresAt || this.tripOverrideExpiresAt > now);
  if (overrideActive) return true;

  const eff = effectiveCountFrom(this, method);
  const limit = Number(method === 'cash' ? (this.cashTripLimit ?? 3) : (this.payfastTripLimit ?? 3));
  return eff < limit;
};
