...
if (ride.status === 'accepted' && ride.pickup?.lat && ride.pickup?.lng) {
  ...
  if (dMeters <= 35) {
    ...
    const COOLDOWN_MS = 20 * 1000; // 20s anti-burst (kept for UI socket)
    ...
    if (firstTime || (ride.arrivedNotified && cooled)) {
      try {
        // Tell code downstream whether this is the first arrival
        driverEvents.emit('ride:arrived', {
          driverId: chatId,
          rideId: String(ride._id),
          firstTime
        });

        // Always update UI viewers
        io.emit(`ride:${ride._id}:arrived`);
      } finally {
        // If it wasn't the first time, refresh cooldown stamp
        if (!firstTime) {
...
driverEvents.on('ride:arrived', async ({ rideId, firstTime = false }) => {
  try {
    ...
    // ✅ Send rider the DM ONLY the first time we mark arrivedNotified=true
    if (firstTime) {
    ...
    // We still emit to sockets each time so the web UI can react, but it won’t DM again
